...
- **Geração de Highlights**: GPT-4 via Pica API identifica os melhores momentos
...
# Edite .env.local e adicione suas chaves da Pica
...
```env
# Pica API - Transcrição via Whisper e Highlights via GPT-4
...
Acesse [picaos.com](https://picaos.com) e obtenha suas chaves de API.
...
│       └── highlights/route.ts  # API GPT-4 via Pica
├── components/
│   ├── ui/                      # shadcn/ui
│   ├── upload/                  # Dropzone
│   ├── audio/                   # Player
...
Gera highlights a partir dos segmentos de transcrição via GPT-4 (Pica API).
