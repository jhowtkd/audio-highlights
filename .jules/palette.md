...
## 2026-06-16 - Replaced Native Window Confirm Dialogs

**UX Problem:** The application used native browser `window.confirm()` and `confirm()` dialogs for destructive actions like resetting a project or retranscribing an audio file. This blocked the main thread, looked jarring, and was inconsistent with the app's custom UI. Deleting a project from the task list lacked a confirmation entirely.
**Learning:** Native confirm dialogs degrade the perceived quality of the web application and interrupt the user flow abruptly. Destructive actions without any confirmation (like deleting a task) can lead to accidental data loss.
**Solution:** Created a reusable, accessible `<ConfirmDialog>` component using the existing Radix UI dialog primitives. Refactored all instances of `window.confirm` to use this new component with appropriate state management, and added a confirmation step for the project deletion action.
**Pattern:** For destructive actions in this application (resetting, deleting, retranscribing), ALWAYS use the `<ConfirmDialog>` component instead of `window.confirm`. Ensure the `confirmVariant` is set to `"destructive"` when appropriate to visually warn the user.
