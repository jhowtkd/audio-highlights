...
## 2025-02-28 - Destructive Action Confirmation

**UX Problem:** Destructive actions like "Delete project" and "Retranscribe" (which deletes current results) were using raw browser `confirm()` dialogs or lacked proper UX flows.

**Learning:** Native browser `confirm()` dialogs are jarring, block the main thread, and don't match the application's design system. Replacing them with custom React dialogs provides a more cohesive and professional experience.

**Solution:** Integrated shadcn/ui's `AlertDialog` component to replace native `confirm()` calls on the `TaskCard` component.

**Pattern:** For destructive or irreversible actions in this app:
1. Always use a custom `AlertDialog` component, not native `window.confirm()`.
2. The dialog must have a clear title (e.g., "Excluir projeto?").
3. The description must explain the consequences (e.g., "Esta ação não poderá ser desfeita").
4. The confirmation button should clearly state the action (e.g., "Excluir", not just "OK").
5. The confirmation button should use an appropriate variant (e.g., `destructive` or colored `bg-red-600` for deletes) to warn the user visually.
