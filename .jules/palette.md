...
## 2026-07-16 - Icon-Only Button Accessibility\n\n**UX Problem:** Multiple icon-only buttons across components (`highlight-card`, `cut-suggestion-card`, `task-card`, `dropzone`) were relying on the `title` attribute for accessibility or lacked proper labeling, while their inner SVG icons were visible to screen readers.\n**Learning:** The `title` attribute is inconsistently supported by screen readers and does not provide robust accessibility for icon-only interactive elements. Additionally, SVGs without `aria-hidden` can cause noise.\n**Solution:** Replaced `title` attributes with `aria-label` on the `<button>` elements and added `aria-hidden="true"` to the inner `lucide-react` icons across affected components.\n**Pattern:** For this design system, ALL icon-only buttons must have: 1. A descriptive `aria-label` on the button itself. 2. `aria-hidden="true"` on the inner SVG/Icon component.
