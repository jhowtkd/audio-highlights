...
## 2026-02-28 - Missing confirmation on destructive actions

**UX Problem:** Destructive actions like deleting a task or clearing completed tasks were instantaneous, causing accidental deletions.
**Learning:** Destructive operations without a confirmation prompt can frustrate users if they accidentally click them.
**Solution:** Added `window.confirm` to act as a safety net before performing these destructive actions, matching the pattern used for retranscribing files. Also added missing `aria-label` and `aria-hidden` attributes to icon-only buttons for accessibility.
**Pattern:** For destructive operations, always ensure a confirmation mechanism exists to prevent irreversible accidents.
