...
## 2024-06-28 - Destructive Action Confirmation Dialogs

**UX Problem:** The app was using native `window.confirm()` dialogs for destructive actions (resetting projects, retranscribing files, deleting tasks). These block the main thread and lack styling consistent with the app's design system.
**Learning:** Using native browser dialogs creates a jarring, unstyled experience and blocks UI interactivity. Users expect confirmation dialogs to match the design system and provide clear, contextual information.
**Solution:** Replaced all instances of `window.confirm()` and `confirm()` with a custom, accessible `<ConfirmDialog>` component built on `@radix-ui/react-dialog`.
**Pattern:** For destructive actions:
1. Never use native `window.confirm()`.
2. Import and use `<ConfirmDialog>` from `@/components/ui/confirm-dialog`.
3. Use state to manage dialog visibility.
4. Provide clear titles, messages, and appropriate button variants (e.g., `variant="destructive"` for deletions).
