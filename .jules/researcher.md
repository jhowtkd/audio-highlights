...
## 2026-03-01 - Client-Side Audio Normalization Research\n\n**Research Topic:** Real-time Client-Side Audio Normalization\n\n**Finding:**\nWeb Audio API provides `DynamicsCompressorNode` which can be used to normalize audio volume directly in the browser during playback without any server roundtrips or backend FFmpeg processing. The key caveat is `createMediaElementSource()` can only be called once per `<audio>` element, so tracking the initialized source using a `useRef` is crucial to prevent `InvalidStateError` upon component re-renders or state changes.\n\n**Decision:**\nPropose implementing a client-side Audio Normalization feature using native Web Audio API in the `AudioPlayer` component.\n\n**Learning:**\nNative browser APIs like Web Audio are incredibly powerful and performant, offering zero-latency audio manipulation without any added bundle size. For features like volume leveling, this is drastically more efficient than server-side re-encoding, providing instant feedback to the user.\n\n**Resources:**\n- https://developer.mozilla.org/en-US/docs/Web/API/DynamicsCompressorNode\n- research/proposals/client-side-audio-normalization.md
