...
## 2025-06-01 - Media Terminology and Action Labels

**Copy Type:** Dialog Title, Dropzone Copy, Submit Button Label
**Original:** "Adicionar novo áudio", "Enviando áudio...", "Arraste seu áudio aqui", "Áudio curto", "Iniciar Transcrição"
**Changed To:** "Novo projeto", "Enviando arquivo...", "Arraste seu arquivo aqui", "Arquivo curto", "Iniciar processamento"
**Learning:** The application accepts video files as well as audio files, making "áudio" too restrictive and potentially confusing for users uploading video. Additionally, "Iniciar Transcrição" describes only the first step of a multi-step pipeline (transcription -> analysis -> highlights generation). Using "Projeto", "Arquivo", and "Processamento" better aligns with the actual capabilities and mental model of the user.
**Rule:** For this application, ALWAYS use 'Projeto' or 'Processamento' for workflows and results, and 'Arquivo' for uploads. Avoid restrictive media terms like 'Áudio' and overly specific action labels like 'Transcrição' when referring to the entire pipeline.
