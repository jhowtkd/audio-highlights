...
## 2026-05-23 - Inclusive Upload Terminology

**Copy Type:** Empty State / Upload text
**Original:** "Enviando áudio...", "Áudio curto", "Arraste seu áudio aqui"
**Changed To:** "Enviando arquivo...", "Arquivo curto", "Arraste seu arquivo aqui"
**Learning:** Using restrictive media terms like "áudio" creates confusion when the application also supports video files. Using inclusive terms like "arquivo" (file) ensures the UI reflects the actual capabilities of the platform and doesn't mislead users into thinking they can only upload audio files.
**Rule:** Avoid restrictive media terms (like "Áudio") for file upload areas and general processing states when the platform supports multiple formats (like video). Use "Arquivo" (File) instead.
