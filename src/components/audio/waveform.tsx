    ...
    useEffect(() => {
        ...
        const generateWaveform = async () => {
            ...
            try {
                ...
                // Optimization: Downsample by using a stride to avoid processing every sample
                // Limits processing to max 100 points per block, reducing O(N) to O(1)
                const maxSamplesPerBlock = 100;
                const stride = Math.max(1, Math.floor(blockSize / maxSamplesPerBlock));
