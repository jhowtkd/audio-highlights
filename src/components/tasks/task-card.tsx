...
export function TaskCard({ task }: TaskCardProps) {
    ...
    const [dialogAction, setDialogAction] = useState<'delete' | 'retranscribe' | null>(null);
    ...
    return (
        <div className={cn(
                    ...
                    <div className="flex items-center gap-2 mt-4">
                        ...
                        {(task.status === 'completed' || task.status === 'error' || task.status === 'pending') && (
                            <Button
                                size="sm"
                                variant="ghost"
                                onClick={() => setDialogAction('retranscribe')}
            ...
            <ConfirmDialog
                open={dialogAction === 'retranscribe'}
            ...
            <ConfirmDialog
                ...
                onOpenChange={(open) => !open && setDialogAction(null)}
                title="Excluir projeto?"
                message="Tem certeza que deseja excluir este projeto? Esta ação não pode ser desfeita e todos os dados e highlights gerados serão perdidos."
