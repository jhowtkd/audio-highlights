...
export function useTaskQueue() {
    ...
    // Memoize the calculation of task counts to prevent re-computation on every render.
    // Performance: Reduces array iteration from O(3N) (three `.filter()` calls) to O(N) (one `.reduce()` pass),
    // and eliminates the creation of intermediate arrays.
