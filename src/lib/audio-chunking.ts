...
export async function processLargeAudioWithFFmpeg(
    ...
    // Optimize merging: Pre-allocate arrays to their final size to eliminate dynamic resizing overhead,
    // and use a classic for-loop for maximum performance in V8.
    let totalSegments = 0;
    ...
    for (let i = 0; i < results.length; i++) {
        ...
        const rSegments = r.segments;
        for (let j = 0; j < rSegments.length; j++) {
            allSegments[segmentIndex++] = rSegments[j];
        }
    }
